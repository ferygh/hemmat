...
class IncomeController extends \BaseController {
	...
	public function view($id){
		...
		foreach($states as $item){
			$info_states[] = [
				'title'=>$item->title,
				'count'=> Person::where('state_id','=', $item->id)
								->where('income_id','=',$id)->count()
			];
		...
		foreach($aids as $item) {
			$info_aids[] = [
				'title' => $item->title,
				'count' => Person::where('aid_id', '=', $item->id)->
								   where('income_id', '=', $id)->count()
			];
		}
		return View::make('income.show')->with(['persons'=>$persons, 'term'=>$term,
						  'states'=>$states, 'incomes'=>$incomes, 'insurances'=>$insurances,
						  'aids'=>$aids, 'info_states'=>$info_states, 'info_incomes'=>$info_incomes,
						  'info_insurances'=>$info_insurances, 'info_aids'=>$info_aids]);
	}
